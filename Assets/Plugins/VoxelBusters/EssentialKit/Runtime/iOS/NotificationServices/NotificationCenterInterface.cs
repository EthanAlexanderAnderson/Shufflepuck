...
namespace VoxelBusters.EssentialKit.NotificationServicesCore.iOS
{
    public sealed class NotificationCenterInterface : NativeNotificationCenterInterfaceBase, INativeNotificationCenterInterface
    {
        ...
        { 
            ...
            {
                callback(false, new Error(description: "The operation could not be completed as request object was already scheduled"));
                return;
            }
            ...
            var     contentPtr  = nativeContent?.Pointer ?? IntPtr.Zero;
            var     triggerPtr  = nativeTrigger?.Pointer ?? IntPtr.Zero;
        ...
        private static void HandleRequestAuthorizationCallback(UNAuthorizationStatus status, NativeError error, IntPtr tagPtr)
        {
            ...
            try
            {
                ...
                var errorObj = error.Convert(NotificationServicesError.kDomain);
        ...
        private static void HandleGetSettingsCallback(ref UNNotificationSettingsData settings, IntPtr tagPtr)
        {
            ...
            try
            {
                ...
                var     unitySettings   = new NotificationSettingsInternal(
                    ...
                    exactTimingSetting: NotificationSettingStatus.Enabled); //This is enabled by default on iOS
        ...
        private static void HandleScheduleNotificationCallback(NativeError error, IntPtr tagPtr)
        {
            var     tagHandle   = GCHandle.FromIntPtr(tagPtr);
            ...
            try
            {
                // send result
                var errorObj = error.Convert(NotificationServicesError.kDomain);
                ((ScheduleNotificationInternalCallback)tagHandle.Target).Invoke((errorObj == null), errorObj);
            }
            catch (Exception exception)
            {
                DebugLogger.LogException(EssentialKitDomain.Default, exception);
        ...
        private static void HandleGetScheduledNotificationsCallback(ref NativeArray arrayPtr, NativeError error, IntPtr tagPtr)
        {
            var     tagHandle   = GCHandle.FromIntPtr(tagPtr);
        ...
        private static void HandleGetDeliveredNotificationsCallback(ref NativeArray arrayPtr, NativeError error, IntPtr tagPtr)
        {
        ...
        private static void HandleRegisterForRemoteNotificationsCallback(string deviceToken, NativeError error, IntPtr tagPtr)
        {
            var     tagHandle   = GCHandle.FromIntPtr(tagPtr);
