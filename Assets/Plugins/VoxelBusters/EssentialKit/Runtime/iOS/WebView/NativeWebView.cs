...
namespace VoxelBusters.EssentialKit.WebViewCore.iOS
{
    public sealed class NativeWebView : NativeWebViewBase, INativeWebView
    {
        ...
        private static void HandleOnShowNativeCallback(IntPtr nativePtr, NativeError error)
        {
            var     owner       = NativeInstanceMap.GetOwner<NativeWebView>(nativePtr);
            Assert.IsPropertyNotNull(owner, "owner");

            // invoke event
            var     errorObj    = error.Convert(WebViewError.kDomain);
            owner.SendShowEvent(errorObj);
        }

        [MonoPInvokeCallback(typeof(WebViewNativeCallback))]
        private static void HandleOnHideNativeCallback(IntPtr nativePtr, NativeError error)
        {
            var     owner       = NativeInstanceMap.GetOwner<NativeWebView>(nativePtr);
            Assert.IsPropertyNotNull(owner, "owner");
        ...
        [MonoPInvokeCallback(typeof(WebViewNativeCallback))]
        private static void HandleLoadStartNativeCallback(IntPtr nativePtr, NativeError error)
        {
            var     owner       = NativeInstanceMap.GetOwner<NativeWebView>(nativePtr);
            Assert.IsPropertyNotNull(owner, "owner");
        ...
        [MonoPInvokeCallback(typeof(WebViewNativeCallback))]
        private static void HandleLoadFinishNativeCallback(IntPtr nativePtr, NativeError error)
        {
            var     owner       = NativeInstanceMap.GetOwner<NativeWebView>(nativePtr);
            Assert.IsPropertyNotNull(owner, nameof(owner));
        ...
        private static void HandleRunJavaScriptNativeCallback(IntPtr nativePtr, string result, NativeError error, IntPtr tagPtr)
        {
