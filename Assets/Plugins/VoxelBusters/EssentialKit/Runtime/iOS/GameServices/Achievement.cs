...
namespace VoxelBusters.EssentialKit.GameServicesCore.iOS
{
    internal sealed class Achievement : AchievementBase
    {
        ...
        private static void HandleLoadAchievementsCallbackInternal(ref NativeArray nativeArray, NativeError error, IntPtr tagPtr)
        {
        ...
        private static void HandleReportProgressCallbackInternal(NativeError error, IntPtr tagPtr)
        {
            var     tagHandle   = GCHandle.FromIntPtr(tagPtr);
            ...
            try
            {
                // send result
                var     errorObj        = error.Convert(GameServicesError.kDomain);
                ((ReportAchievementProgressInternalCallback)tagHandle.Target)((errorObj == null), errorObj);
            }
            catch (Exception exception)
            {
                DebugLogger.LogException(EssentialKitDomain.Default, exception);
