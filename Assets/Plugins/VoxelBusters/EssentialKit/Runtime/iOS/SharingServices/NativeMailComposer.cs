...
namespace VoxelBusters.EssentialKit.SharingServicesCore.iOS
{
    internal sealed class NativeMailComposer : NativeMailComposerBase, INativeMailComposer
    {
        ...
        private static void HandleMailComposerClosedCallbackInternal(IntPtr nativePtr, MFMailComposeResult result, NativeError error)
        {
            var     owner   = NativeInstanceMap.GetOwner<NativeMailComposer>(nativePtr);
            Assert.IsPropertyNotNull(owner, nameof(owner));

            // send result
            var     errorObj    = error.Convert(SharingServicesError.kDomain);
