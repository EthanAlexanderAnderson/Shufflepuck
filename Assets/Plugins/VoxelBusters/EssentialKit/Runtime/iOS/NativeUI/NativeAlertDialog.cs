...
{
    internal sealed partial class NativeAlertDialog : NativeAlertDialogInterfaceBase, INativeAlertDialogInterface
    {
        #region Constructors
        ...
        {
            NativeTextInputFieldOptions nativeOptions = default(NativeTextInputFieldOptions);

            if (options != null)
            {
                nativeOptions = new NativeTextInputFieldOptions()
                {
                    Text                = options.Text,
                    PlaceholderText     = options.PlaceholderText,
                    IsSecured           = options.IsSecured ? 1 : 0,
                    KeyboardInputType   = options.KeyboardInputType
                };    
            }
            
            AlertControllerBinding.NPAlertControllerAddTextInputField(AddrOfNativeObject(), nativeOptions);
        }
        ...
        private static void HandleAlertActionSelectCallbackInternal(IntPtr nativePtr, int selectedButtonIndex, ref NativeArray inputValues)
        {
            var     owner   = NativeInstanceMap.GetOwner<NativeAlertDialog>(nativePtr);
            var     inputValuesArray   = MarshalUtility.CreateStringArray(inputValues.Pointer, inputValues.Length);

            Assert.IsPropertyNotNull(owner, "owner");

            owner.SendButtonClickEvent(selectedButtonIndex, inputValuesArray);
        }
