...
{
    public class NativeNotificationsRequestListener : AndroidJavaProxy
    {
        #region Delegates

        public delegate void OnFailureDelegate(NativeErrorInfo error);
        public delegate void OnSuccessDelegate(NativeList<NativeNotification> notifications);
        ...
        public NativeNotificationsRequestListener() : base("com.voxelbusters.essentialkit.notificationservices.INotificationServices$INotificationsRequestListener")
        {
        }
        ...
        public void onSuccess(AndroidJavaObject notifications)
        {
#if NATIVE_PLUGINS_DEBUG_ENABLED
            DebugLogger.Log("[Proxy : Callback] : " + "onSuccess"  + " " + "[" + "notifications" + " : " + notifications +"]");
#endif
            if(onSuccessCallback != null)
            {
                onSuccessCallback(new NativeList<NativeNotification>(notifications));
            }
        }
