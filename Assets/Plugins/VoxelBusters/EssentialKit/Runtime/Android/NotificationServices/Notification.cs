...
#region Base class implementation
        ...
        protected override INotificationTrigger GetTriggerInternal()
        {
            if(m_trigger == null)
            {
                ...
                if (trigger != null)
                {
                    ...
                    else if(m_instance.HasCalendarTimeTrigger())
                    {
                        NativeCalendarNotificationTrigger nativeCalendarNotificationTrigger = new NativeCalendarNotificationTrigger(trigger);
                        var dateComponents = GetDateComponents(nativeCalendarNotificationTrigger);
                        m_trigger = new CalendarNotificationTrigger(dateComponents, nativeCalendarNotificationTrigger.Repeat, nativeCalendarNotificationTrigger.GetNextTriggerDate().GetDateTime());                                                
                    }
        ...
        private DateComponents GetDateComponents(NativeCalendarNotificationTrigger nativeTrigger)
        {
            DateComponents dateComponents = new DateComponents();

            dateComponents.Nanosecond   = nativeTrigger.GetNanosecond();
            dateComponents.Second       = nativeTrigger.GetSecond();
            dateComponents.Minute       = nativeTrigger.GetMinute();
            dateComponents.Hour         = nativeTrigger.GetHour();
            dateComponents.Day          = nativeTrigger.GetDay();
            dateComponents.Month        = nativeTrigger.GetMonth();
            dateComponents.Year         = nativeTrigger.GetYear();
            dateComponents.DayOfWeek    = nativeTrigger.GetWeekday();
            dateComponents.WeekOfMonth  = nativeTrigger.GetWeekOfMonth();
            dateComponents.WeekOfYear   = nativeTrigger.GetWeekOfYear();
