...
{
    public class NativeMediaContentSelectionListener : AndroidJavaProxy
    {
        #region Delegates

        public delegate void OnFailureDelegate(NativeErrorInfo error);
        public delegate void OnSuccessDelegate(NativeList<NativeMediaContentBase> assets);
        ...
        public NativeMediaContentSelectionListener() : base("com.voxelbusters.essentialkit.mediaservices.IMediaServices$IMediaContentSelectionListener")
        {
        }
        ...
        public void onSuccess(AndroidJavaObject assets)
        {
#if NATIVE_PLUGINS_DEBUG_ENABLED
            DebugLogger.Log("[Proxy : Callback] : " + "onSuccess"  + " " + "[" + "assets" + " : " + assets +"]");
#endif
            if(onSuccessCallback != null)
            {
                onSuccessCallback(new NativeList<NativeMediaContentBase>(assets));
            }
        }
