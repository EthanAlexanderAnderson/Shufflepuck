...
{
    public class NativeMediaContentRawDataCallback : AndroidJavaProxy
    {
        #region Delegates

        public delegate void OnFailureDelegate(NativeErrorInfo error);
        public delegate void OnSuccessDelegate(NativeBytesWrapper data, string mime);
        ...
        public NativeMediaContentRawDataCallback() : base("com.voxelbusters.essentialkit.mediaservices.IMediaServices$IMediaContentRawDataCallback")
        {
        }
        ...
        public void onSuccess(AndroidJavaObject data, string mime)
        {
#if NATIVE_PLUGINS_DEBUG_ENABLED
            DebugLogger.Log("[Proxy : Callback] : " + "onSuccess"  + " " + "[" + "data" + " : " + data +"]" + " " + "[" + "mime" + " : " + mime +"]");
#endif
            if(onSuccessCallback != null)
            {
                onSuccessCallback(new NativeBytesWrapper(data), mime);
            }
        }
