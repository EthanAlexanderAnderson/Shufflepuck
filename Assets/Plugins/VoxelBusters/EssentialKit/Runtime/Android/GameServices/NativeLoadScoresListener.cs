...
{
    public class NativeLoadScoresListener : AndroidJavaProxy
    {
        #region Delegates

        public delegate void OnFailureDelegate(NativeErrorInfo error);
        public delegate void OnSuccessDelegate(NativeList<NativeGameLeaderboardScore> scores);
        ...
        public NativeLoadScoresListener() : base("com.voxelbusters.essentialkit.gameservices.IGameServices$ILoadScoresListener")
        {
        }
        ...
        public void onSuccess(AndroidJavaObject scores)
        {
#if NATIVE_PLUGINS_DEBUG_ENABLED
            DebugLogger.Log("[Proxy : Callback] : " + "onSuccess"  + " " + "[" + "scores" + " : " + scores +"]");
#endif
            if(onSuccessCallback != null)
            {
                onSuccessCallback(new NativeList<NativeGameLeaderboardScore>(scores));
            }
        }
