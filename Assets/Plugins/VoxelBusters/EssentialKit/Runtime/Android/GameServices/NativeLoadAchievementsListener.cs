...
{
    public class NativeLoadAchievementsListener : AndroidJavaProxy
    {
        #region Delegates

        public delegate void OnFailureDelegate(NativeErrorInfo error);
        public delegate void OnSuccessDelegate(NativeArrayBuffer<NativeGameAchievement> achievements);
        ...
        public NativeLoadAchievementsListener() : base("com.voxelbusters.essentialkit.gameservices.IGameServices$ILoadAchievementsListener")
        {
        }
        ...
        public void onSuccess(AndroidJavaObject achievements)
        {
#if NATIVE_PLUGINS_DEBUG_ENABLED
            DebugLogger.Log("[Proxy : Callback] : " + "onSuccess"  + " " + "[" + "achievements" + " : " + achievements +"]");
#endif
            if(onSuccessCallback != null)
            {
                onSuccessCallback(new NativeArrayBuffer<NativeGameAchievement>(achievements));
            }
        }
