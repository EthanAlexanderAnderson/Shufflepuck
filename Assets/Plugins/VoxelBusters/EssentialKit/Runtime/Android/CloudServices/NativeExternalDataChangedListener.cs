...
namespace VoxelBusters.EssentialKit.CloudServicesCore.Android
{
    public class NativeExternalDataChangedListener : AndroidJavaProxy
    {
        ...
        public delegate void OnChangeDelegate(NativeExternalChangeReason reason, string[] keys);
        ...
        public NativeExternalDataChangedListener() : base("com.voxelbusters.essentialkit.cloudservices.ICloudServices$IExternalDataChangedListener")
        {
        }
        ...
        public void onChange(AndroidJavaObject reason, string[] keys)
        {
#if NATIVE_PLUGINS_DEBUG_ENABLED
            DebugLogger.Log("[Proxy : Callback] : " + "onChange"  + " " + "[" + "reason" + " : " + reason +"]" + " " + "[" + "keys" + " : " + keys +"]");
#endif
            if(onChangeCallback != null)
            {
                onChangeCallback(NativeExternalChangeReasonHelper.ReadFromValue(reason), keys);
            }
        }
