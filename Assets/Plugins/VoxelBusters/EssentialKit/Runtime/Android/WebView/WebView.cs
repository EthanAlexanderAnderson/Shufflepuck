...
#region Constructors
            ...
            : base(isAvailable: true)
        {
            ...
            m_instance.SetViewListener(new NativeWebViewListener()
            {
                ...
                onPageLoadErrorCallback = (failingUrl, error) =>
                {
                    DebugLogger.Log(string.Format("Failing Url : {0} Description : {1}", failingUrl, error));
                    SendLoadFinishEvent(error.Convert(WebViewError.kDomain));
                },
...
#region Base class methods
        ...
        public override void RunJavaScript(string script, RunJavaScriptInternalCallback callback)
        {
            m_instance.EvaluateJavaScriptFromString(script, new NativeEvaluateJavaScriptListener()
            {
                ...
                onFailureCallback = (error)     => callback(null, error.Convert(WebViewError.kDomain))
            });
        }
