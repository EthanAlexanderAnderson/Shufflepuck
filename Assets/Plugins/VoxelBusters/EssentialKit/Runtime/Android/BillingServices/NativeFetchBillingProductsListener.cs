...
{
    public class NativeFetchBillingProductsListener : AndroidJavaProxy
    {
        #region Delegates

        public delegate void OnFailureDelegate(NativeErrorInfo errorInfo);
        public delegate void OnSuccessDelegate(NativeList<NativeBillingProduct> products, NativeArrayBuffer<string> invalidIds);
        ...
        public NativeFetchBillingProductsListener() : base("com.voxelbusters.essentialkit.billingservices.common.interfaces.IFetchBillingProductsListener")
        {
        }
        ...
        public void onSuccess(AndroidJavaObject products, AndroidJavaObject invalidIds)
        {
#if NATIVE_PLUGINS_DEBUG_ENABLED
            DebugLogger.Log("[Proxy : Callback] : " + "onSuccess"  + " " + "[" + "products" + " : " + products +"]" + " " + "[" + "invalidIds" + " : " + invalidIds +"]");
#endif
            if(onSuccessCallback != null)
            {
                onSuccessCallback(new NativeList<NativeBillingProduct>(products), new NativeArrayBuffer<string>(invalidIds));
            }
        }
