...
namespace VoxelBusters.EssentialKit.AddressBookCore.Android
{
    public class NativeReadContactsListener : AndroidJavaProxy
    {
        ...
        public delegate void OnSuccessDelegate(NativeList<NativeAddressBookContact> contacts, int nextOffset);
        ...
        public void onSuccess(AndroidJavaObject contacts, int nextOffset)
        {
#if NATIVE_PLUGINS_DEBUG_ENABLED
            DebugLogger.Log("[Proxy : Callback] : " + "onSuccess"  + " " + "[" + "contacts" + " : " + contacts +"]" + " " + "[" + "nextOffset" + " : " + nextOffset +"]");
#endif
            if(onSuccessCallback != null)
            {
                onSuccessCallback(new NativeList<NativeAddressBookContact>(contacts), nextOffset);
            }
        }
