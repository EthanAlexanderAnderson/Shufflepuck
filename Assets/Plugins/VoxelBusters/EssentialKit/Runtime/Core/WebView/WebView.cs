...
{
    /** @defgroup WebView Web View
    *   @brief Provides a cross-platform interface to display web contents inside your application.
    */
    ...
    public sealed class WebView : NativeFeatureBehaviour
    {
        ...
        {
            Assert.IsArgNotNull(globalSettings, nameof(globalSettings));
        ...
        {
            return CreateInstanceInternal<WebView>("WebView", new object[] { settings }); //@@ Passing array object isn't required but just to make aware of the ambiguity when passing null to params parameter so that on the receiving end it's considered accordingly (as array with null element if null is passed).
        }
        ...
        protected override void AwakeInternal(object[] args)
        {
            ...
            // Configure interface
            Settings            = (args.IsNullOrEmpty() || args[0] == null) ? GlobalSettings : args[0] as WebViewUnitySettings;
            m_nativeInterface   = NativeFeatureActivator.CreateInterface<INativeWebView>(ImplementationSchema.WebView, true);
        ...
        public void LoadHtmlString(string htmlString, URLString? baseURL = null)
        {
            ...
            if (null == htmlString)
            {
                DebugLogger.LogWarning(EssentialKitDomain.Default, "Html string is null.");
                return;
            }
            if (baseURL.HasValue && !baseURL.Value.IsValid)
            {
                DebugLogger.LogWarning(EssentialKitDomain.Default, "Specified base url is invalid.");
                return;
            }
        ...
        public void LoadData(byte[] data, string mimeType, string textEncodingName, URLString? baseURL = null)
        {
            ...
            if (null == data)
            {
                DebugLogger.LogWarning(EssentialKitDomain.Default, "Data array is null.");
                return;
            }
            ...
            {
                DebugLogger.LogWarning(EssentialKitDomain.Default, "Text encoding name is null.");
                return;
            }
            if (baseURL.HasValue && !baseURL.Value.IsValid)
            {
        ...
        public void RunJavaScript(string script, EventCallback<WebViewRunJavaScriptResult> callback)
        {
            ...
            if (script == null)
            {
                DebugLogger.LogWarning(EssentialKitDomain.Default, "Script is null.");
                return;
            }
            ...
            try
            {
                ...
                m_nativeInterface.RunJavaScript(script, (jsResult, jsError) =>
                {
                    ...
                    var     result  = new WebViewRunJavaScriptResult(jsResult);
