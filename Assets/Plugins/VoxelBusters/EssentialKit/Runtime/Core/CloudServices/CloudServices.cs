...
{
    /** @defgroup CloudServices Cloud Services
    *   @brief Provides a cross-platform interface to sync information across various devices by storing it in the cloud.
    */
    ...
    public static class CloudServices
    {
        ...
        /// </summary>
        /// @note This event is triggered ONLY if there are changes between local and cloud data values.
        ...
        public static void Initialize(CloudServicesUnitySettings settings)
        {
            ...
            // Configure interface
            s_nativeInterface       = NativeFeatureActivator.CreateInterface<INativeCloudServicesInterface>(ImplementationSchema.CloudServices, true);
        ...
        /// <summary>
        /// Returns <c>true</c> if key exists in the data snapshot.
        /// </summary>
        /// <param name="key">A string used to identify the value stored in the current data snapshot.</param>
        /// <returns><c>true</c> if key exists in the data snapshot, otherwise <c>false</c>.</returns>
        public static bool HasKey(string key)
        {
            ...
            {
                // make request
                return s_nativeInterface.HasKey(key);
            }
            catch (Exception exception)
            {
                DebugLogger.LogException(EssentialKitDomain.Default, exception);
