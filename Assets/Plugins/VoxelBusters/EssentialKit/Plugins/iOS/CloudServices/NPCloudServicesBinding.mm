...
- (void)sendUserAccountInfo
{
    ...
    // send callback
    _userChangeCallback(accountData, NPCreateError(nil));
...
NPBINDING DONTSTRIP bool NPCloudServicesHasKey(const char* key)
{
    return [[NSUbiquitousKeyValueStore defaultStore] objectForKey:NPCreateNSStringFromCString(key)] != nil;
}
