...
{
    NSURL *url = [NSURL fileURLWithPath:[NSTemporaryDirectory() stringByAppendingString:NPCreateNSStringFromCString((const char*)data.fileNamePtr)]];
    NSData *rawData = [NSData dataWithBytes:data.dataArrayPtr length:data.dataArrayLength];
    [rawData writeToURL:url atomically:YES];
    
    NPShareSheetWrapper*    shareSheet  = (__bridge NPShareSheetWrapper*)nativePtr;
    [shareSheet addItem:url];
}
