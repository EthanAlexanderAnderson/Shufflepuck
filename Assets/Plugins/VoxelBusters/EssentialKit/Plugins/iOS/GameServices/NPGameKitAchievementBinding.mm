...
NPBINDING DONTSTRIP void NPAchievementLoadAchievements(void* tagPtr)
{
    [GKAchievement loadAchievementsWithCompletionHandler:^(NSArray<GKAchievement*>* _Nullable achievements, NSError* _Nullable error) {
        ...
        _loadAchievementsCallback(nativeArray, NPCreateError(CreateFromError(error, Domain, GameServicesErrorCodeUnknown)), tagPtr);
...
NPBINDING DONTSTRIP void NPAchievementReportProgress(void* achievementPtr, void* tagPtr)
{
    if(![[GKLocalPlayer localPlayer] isAuthenticated]) {
        ...
        _reportAchievementCallback(NPCreateError(ErrorWithDomain(Domain, GameServicesErrorCodeNotAuthenticated, @"Player not authenticated.")), tagPtr);
    ...
    [GKAchievement reportAchievements:@[achievement] withCompletionHandler:^(NSError * _Nullable error) {
        // send data
        //TODO: Find different possible errors
        _reportAchievementCallback(NPCreateError(CreateFromError(error, Domain, GameServicesErrorCodeUnknown)), tagPtr);
    }];
}

NPBINDING DONTSTRIP void NPAchievementShowView(void* tagPtr)
{
    GKGameCenterViewController* gameCenterVC    = [[GKGameCenterViewController alloc] initWithState:GKGameCenterViewControllerStateAchievements];
